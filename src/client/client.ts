...
const chaseCam: THREE.Object3D = new THREE.Object3D()
scene.add(chaseCam)
...
const recentWinnersTable: HTMLTableElement = document.getElementById('recentWinnersTable') as HTMLTableElement;
...
function lockChangeAlert() {
    if (document.pointerLockElement === renderer.domElement || (document as any).mozPointerLockElement === renderer.domElement) {
    ...
    (object) => {
        object.traverse(function (child) {
            if ((<THREE.Mesh>child).isMesh) {
                (<THREE.Mesh>child).material = jewelMaterial
            }
        })
        jewel = object
        scene.add(jewel);
...
const sphereGeometry: THREE.SphereBufferGeometry = new THREE.SphereBufferGeometry(1, 24, 24)
const cubeGeometry: THREE.BoxBufferGeometry = new THREE.BoxBufferGeometry(2, 2, 2)

const sphereMaterial: THREE.MeshBasicMaterial = new THREE.MeshBasicMaterial({
    map: texture,
...
const myMaterial: THREE.MeshPhongMaterial = new THREE.MeshPhongMaterial({
    map: texture,
    reflectivity: .66,
    color: 0xffffff,
    flatShading: false,
...
const jewelMaterial: THREE.MeshMatcapMaterial = new THREE.MeshMatcapMaterial({
...
let myId: string = ""
let timestamp = 0
const players: { [id: string]: THREE.Mesh } = {}
const obstacles: { [id: string]: THREE.Mesh } = {}
